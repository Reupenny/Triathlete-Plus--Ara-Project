...
function NewSession({ onHide, controller, setNewSession, session, sessionData }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (sessionData) {
            ...
            controller.editTrainingSession(session.sessionID, formData);
        } else {
        ...
        <>
            ...
            <div className="box" id="newSession">
                ...
                <form id='newSessionForm' onSubmit={handleSubmit}>
                    {selectedSport === 'running' && <RunningForm onFormChange={handleFormChange} formData={formData} />}
                    {selectedSport === 'swimming' && <SwimmingForm onFormChange={handleFormChange} formData={formData} />}
                    {selectedSport === 'cycling' && <CyclingForm onFormChange={handleFormChange} formData={formData} />}
...
function MainPanel({ controller, onEditSession }) {
    ...
    const handleEdit = (sessionID) => {
        controller.findTrainingSessionByID(sessionID)
            .then(sessionData => {
                if (sessionData) {
                    onEditSession(sessionData); // Call the passed prop to open modal in TriathlonView
                } else {
                    toast.error("Could not find session data to edit.");
                }
            })
            .catch(error => {
                                ...
                                return (
                                    <tr key={index} data-sport={entry.sportType}>
                                        ...
                                        <td className="more-cell">
                                            ...
                                            <i className='icon icon-edit' onClick={() => handleEdit(entry.sessionID)}></i>
...
function TriathlonView({ controller }) {
    ...
    return (
        <>
            ...
            {loggedIn ? (<MainPanel controller={controller} onEditSession={handleOpenEditModal} />) : null}
        </>
    );
}
